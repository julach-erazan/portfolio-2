import { bA as createAssigner, bB as isPrototype, aU as isArrayLike, bk as copyObject, aY as assignValue, ar as baseRest, as as isArrayLikeObject, bp as nodeUtil, av as isEmpty, aV as isArray, bd as isObjectLike, be as baseGetTag, aW as identity, au as isFunction, bb as baseUnary, ba as baseAssignValue, aQ as isObject } from "./mermaid.core-DCmG7zLC.mjs";
import { m as keys, b as baseFlatten, S as SetCache, f as forEach, w as arrayIncludes, i as isUndefined, v as values, x as baseIndexOf, a as arrayMap, y as getAllKeysIn, d as baseIteratee, z as noop, r as reduce, A as arrayFilter, B as baseFilter, g as filter, o as baseEach, n as baseUniq, C as arraySome, D as cacheHas } from "./_baseUniq-BjxFE0Gq.mjs";
import { h as has, f as flatten, m as map, j as toInteger, e as basePickBy, l as last, d as defaults, g as find, a as min } from "./_basePickBy-Boao_KiU.mjs";
import { c as clone } from "./clone-C7dC8Rzl.mjs";
import require$$0$4 from "util";
import require$$2__default__default from "path";
import require$$1$2 from "os";
import require$$1$4 from "crypto";
import require$$0 from "net";
import { a as getAugmentedNamespace } from "./server.mjs";
...
class AbstractAstReflection {
  ...
  isInstance(node2, type) {
    return isAstNode(node2) && this.isSubtype(node2.$type, type);
  }
...
function linkContentToContainer(node2, options = {}) {
...
function streamContents(node2, options) {
  if (!node2) {
...
function streamAllContents(root, options) {
  ...
  return new TreeStreamImpl(root, (node2) => streamContents(node2, options));
}
function streamAst(root, options) {
  if (!root) {
    throw new Error("Root node must be an AstNode.");
  }
  return new TreeStreamImpl(root, (node2) => streamContents(node2, options), { includeRoot: true });
}
    ...
    while (state.keyIndex < state.keys.length) {
      ...
      if (!property.startsWith("$")) {
        ...
        if (isReference(value) || isMultiReference(value)) {
          state.keyIndex++;
          return { done: false, value: { reference: value, container: node2, property } };
        } else if (Array.isArray(value)) {
          while (state.arrayIndex < value.length) {
            const index = state.arrayIndex++;
            const element = value[index];
            if (isReference(element) || isMultiReference(value)) {
              return { done: false, value: { reference: element, container: node2, property, index } };
            }
          }
          state.arrayIndex = 0;
        }
...
function assignMandatoryProperties(reflection2, node2) {
  const typeMetaData = reflection2.getTypeMetaData(node2.$type);
...
function streamCst(node2) {
  return new TreeStreamImpl(node2, (element) => {
...
function getPreviousNode(node2, hidden = true) {
  while (node2.container) {
    const parent = node2.container;
    let index = parent.content.indexOf(node2);
    while (index > 0) {
      index--;
...
class TerminalRegExpVisitor extends BaseRegExpVisitor {
  ...
  visitCharacter(node2) {
    const char = String.fromCharCode(node2.value);
  ...
  visitSet(node2) {
    if (!this.multiline) {
      const set = this.regex.substring(node2.loc.begin, node2.loc.end);
...
function ruleDfs(rule, visitedSet, allTerminals) {
  ...
  streamAllContents(rule).forEach((node2) => {
    if (isRuleCall(node2) || allTerminals) {
...
function findNodesForProperty(node2, property) {
  if (!node2 || !property) {
    return [];
  }
  return findNodesForPropertyInternal(node2, property, node2.astNode, true);
}
function findNodeForProperty(node2, property, index) {
  if (!node2 || !property) {
    return void 0;
  }
  const nodes = findNodesForPropertyInternal(node2, property, node2.astNode, true);
  if (nodes.length === 0) {
    return void 0;
  }
  if (index !== void 0) {
...
function findNodesForPropertyInternal(node2, property, element, first2) {
  if (!first2) {
  ...
  if (isCompositeCstNode(node2) && node2.astNode === element) {
    return node2.content.flatMap((e) => findNodesForPropertyInternal(e, property, element, false));
  }
  return [];
}
function findNodeForKeyword(node2, keyword, index) {
  ...
  const nodes = findNodesForKeywordInternal(node2, keyword, node2?.astNode);
  if (nodes.length === 0) {
    return void 0;
  }
  if (index !== void 0) {
...
function findNodesForKeywordInternal(node2, keyword, element) {
  if (node2.astNode !== element) {
    return [];
  }
...
function serializeProduction(node2) {
  ...
  if (node2 instanceof NonTerminal) {
  ...
  } else if (node2 instanceof Option$1) {
    return {
      type: "Option",
      idx: node2.idx,
      definition: convertDefinition(node2.definition)
    ...
    const serializedTerminal = {
      ...
      label: tokenLabel$1(node2.terminalType),
      idx: node2.idx
    };
    if (isString(node2.label)) {
...
class CharCodeFinder extends BaseRegExpVisitor {
  ...
  visitCharacter(node2) {
    if (includes(this.targetCharCodes, node2.value)) {
      this.found = true;
    }
  }
  visitSet(node2) {
...
function addTerminalToCst(node2, token, tokenTypeName) {
  if (node2.children[tokenTypeName] === void 0) {
    node2.children[tokenTypeName] = [token];
  } else {
    node2.children[tokenTypeName].push(token);
  }
}
...
class CstNodeBuilder {
  ...
  removeNode(node2) {
    const parent = node2.container;
    if (parent) {
      const index = parent.content.indexOf(node2);
      if (index >= 0) {
  ...
  action($type, action) {
    if (!this.isRecording()) {
      ...
      if (action.feature && action.operator) {
        ...
        node2.content.push(last2.$cstNode);
        const newItem = { $type };
        this.stack.push(newItem);
...
class UriTrie {
  ...
  findNode(uri) {
    ...
    return {
      name: node2.name,
      uri: UriUtils.joinPath(URI.parse(uriString), node2.name).toString(),
...
const RefResolving = /* @__PURE__ */ Symbol("RefResolving");
class DefaultLinker {
  constructor(services) {
  ...
  async link(document, cancelToken = cancellationExports.CancellationToken.None) {
    if (this.profiler?.isActive("linking")) {
      ...
      task.start();
      try {
        for (const node2 of streamAst(document.parseResult.value)) {
      ...
      for (const node2 of streamAst(document.parseResult.value)) {
        await interruptAndCheck(cancelToken);
        streamReferences(node2).forEach((ref) => this.doLink(ref, document));
      }
    }
  }
  ...
  buildReference(node2, property, refNode, refText) {
    const linker = this;
    const reference = {
      $refNode: refNode,
      $refText: refText,
      ...
      get ref() {
        if (isAstNode(this._ref)) {
          ...
          this._ref = linkedNode ?? linker.createLinkingError({ reference, container: node2, property }, this._nodeDescription);
        } else if (this._ref === void 0) {
          this._ref = RefResolving;
          const document = findRootNode(node2).$document;
          const refData = linker.getLinkedNode({ reference, container: node2, property });
          ...
          linker.throwCyclicReferenceError(node2, property, refText);
        }
  ...
  buildMultiReference(node2, property, refNode, refText) {
    const linker = this;
    const reference = {
      $refNode: refNode,
      $refText: refText,
      ...
      get error() {
        ...
        if (refs.length > 0) {
          ...
          return this._linkingError = linker.createLinkingError({ reference, container: node2, property });
        }
      }
    };
    return reference;
  }
  throwCyclicReferenceError(node2, property, refText) {
    throw new Error(`Cyclic reference resolution detected: ${this.astNodeLocator.getAstNodePath(node2)}/${property} (symbol '${refText}')`);
  }
...
class DefaultScopeComputation {
  ...
  async collectExportedSymbolsForNode(parentNode, document, children = streamContents, cancelToken = cancellationExports.CancellationToken.None) {
    ...
    for (const node2 of children(parentNode)) {
      await interruptAndCheck(cancelToken);
      this.addExportedSymbol(node2, exports$1, document);
    }
    return exports$1;
  }
  /**
   ...
   */
  addExportedSymbol(node2, exports$1, document) {
    const name = this.nameProvider.getName(node2);
    if (name) {
      exports$1.push(this.descriptions.createDescription(node2, name, document));
    }
  }
  ...
  async collectLocalSymbols(document, cancelToken = cancellationExports.CancellationToken.None) {
    ...
    for (const node2 of streamAllContents(rootNode)) {
      await interruptAndCheck(cancelToken);
      this.addLocalSymbol(node2, document, symbols);
    }
    return symbols;
  }
  /**
   ...
   */
  addLocalSymbol(node2, document, symbols) {
    const container = node2.$container;
    if (container) {
      const name = this.nameProvider.getName(node2);
      if (name) {
        symbols.add(container, this.descriptions.createDescription(node2, name, document));
      }
    }
  }
}
...
class DefaultJsonSerializer {
  ...
  serialize(node2, options) {
    ...
    try {
      ...
      return JSON.stringify(node2, replacer, options?.space);
    } finally {
  ...
  linkNode(node2, root, options, container, containerProperty, containerIndex) {
    for (const [propertyName, item] of Object.entries(node2)) {
      if (Array.isArray(item)) {
        for (let index = 0; index < item.length; index++) {
          const element = item[index];
          if (isIntermediateReference(element)) {
            item[index] = this.reviveReference(node2, propertyName, root, element, options);
          } else if (isAstNode(element)) {
            this.linkNode(element, root, options, node2, propertyName, index);
          }
        }
      } else if (isIntermediateReference(item)) {
        node2[propertyName] = this.reviveReference(node2, propertyName, root, item, options);
      } else if (isAstNode(item)) {
        this.linkNode(item, root, options, node2, propertyName);
      }
    }
    const mutable = node2;
  ...
  getRefNode(root, uri, uriConverter) {
    try {
      ...
      if (fragmentIndex === 0) {
        const node3 = this.astNodeLocator.getAstNode(root, uri.substring(1));
        if (!node3) {
      ...
      const node2 = this.astNodeLocator.getAstNode(document.parseResult.value, uri.substring(fragmentIndex + 1));
      if (!node2) {
    ...
    if (this.profiler?.isActive("validating")) {
      ...
      try {
        ...
        for (const node2 of nodes) {
          ...
          const nodeOptions = this.validateSingleNodeOptions(node2, options);
          if (nodeOptions.validateNode) {
            try {
              const checks = this.validationRegistry.getChecks(node2.$type, options.categories);
...
class DefaultAstNodeDescriptionProvider {
  ...
  createDescription(node2, name, document) {
    const doc = document ?? getDocument(node2);
    name ?? (name = this.nameProvider.getName(node2));
    const path = this.astNodeLocator.getAstNodePath(node2);
    if (!name) {
      throw new Error(`Node at path ${path} has no name.`);
    }
    let nameNodeSegment;
    const nameSegmentGetter = () => nameNodeSegment ?? (nameNodeSegment = toDocumentSegment(this.nameProvider.getNameNode(node2) ?? node2.$cstNode));
    return {
      node: node2,
      name,
      get nameSegment() {
...
function requireMain$1() {
  ...
  (function(exports$1) {
    ...
    const path = require$$2__default__default;
...
function parseJSDoc(node2, start, options) {
  let opts;
  let position;
...
function isJSDoc(node2, options) {
...
class JSDocDocumentationProvider {
  ...
  getDocumentation(node2) {
    const comment = this.commentProvider.getComment(node2);
    if (comment && isJSDoc(comment)) {
      const parsedJSDoc = parseJSDoc(comment);
      return parsedJSDoc.toMarkdown({
        renderLink: (link, display) => {
          return this.documentationLinkRenderer(node2, link, display);
        },
        renderTag: (tag) => {
  ...
  findNameInLocalSymbols(node2, name) {
    const document = getDocument(node2);
...
class DefaultHydrator {
  ...
  setParent(node2, parent) {
    node2.$container = parent;
    return node2;
  }
  ...
  hydrateCstLeafNode(cstNode) {
    ...
    const node2 = new LeafCstNodeImpl(offset, length, {
      start: {
        line: startLine,
...
var initializers = {
  info: /* @__PURE__ */ __name(async () => {
    const { createInfoServices: createInfoServices2 } = await Promise.resolve().then(function() {
      return infoVBDWY6EOCfC6OdiZ;
    });
    const parser = createInfoServices2().Info.parser.LangiumParser;
    parsers.info = parser;
  }, "info"),
  packet: /* @__PURE__ */ __name(async () => {
    const { createPacketServices: createPacketServices2 } = await Promise.resolve().then(function() {
      return packetDYOGHKS2C8PlCULr;
    });
    const parser = createPacketServices2().Packet.parser.LangiumParser;
    parsers.packet = parser;
  }, "packet"),
  pie: /* @__PURE__ */ __name(async () => {
    const { createPieServices: createPieServices2 } = await Promise.resolve().then(function() {
      return pieVRWISCQLDGGTw8T;
    });
    const parser = createPieServices2().Pie.parser.LangiumParser;
    parsers.pie = parser;
  }, "pie"),
  architecture: /* @__PURE__ */ __name(async () => {
    const { createArchitectureServices: createArchitectureServices2 } = await Promise.resolve().then(function() {
      return architecture7HQA4BMRC7LKnTlZ;
    });
    const parser = createArchitectureServices2().Architecture.parser.LangiumParser;
    parsers.architecture = parser;
  }, "architecture"),
  gitGraph: /* @__PURE__ */ __name(async () => {
    const { createGitGraphServices: createGitGraphServices2 } = await Promise.resolve().then(function() {
      return gitGraphG5XIXVHTAjcBNkBb;
    });
    const parser = createGitGraphServices2().GitGraph.parser.LangiumParser;
    parsers.gitGraph = parser;
  }, "gitGraph"),
  radar: /* @__PURE__ */ __name(async () => {
    const { createRadarServices: createRadarServices2 } = await Promise.resolve().then(function() {
      return radarZZBFDIW7Q4vOBeLg;
    });
    const parser = createRadarServices2().Radar.parser.LangiumParser;
    parsers.radar = parser;
  }, "radar"),
  treemap: /* @__PURE__ */ __name(async () => {
    const { createTreemapServices: createTreemapServices2 } = await Promise.resolve().then(function() {
      return treemapGDKQZRPOV6tldQ7S;
    });
...
const infoVBDWY6EOCfC6OdiZ = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  InfoModule,
  createInfoServices
});
const packetDYOGHKS2C8PlCULr = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  PacketModule,
  createPacketServices
});
const pieVRWISCQLDGGTw8T = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  PieModule,
  createPieServices
});
const architecture7HQA4BMRC7LKnTlZ = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  ArchitectureModule,
  createArchitectureServices
});
const gitGraphG5XIXVHTAjcBNkBb = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  GitGraphModule,
  createGitGraphServices
});
const radarZZBFDIW7Q4vOBeLg = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  RadarModule,
  createRadarServices
});
const treemapGDKQZRPOV6tldQ7S = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  TreemapModule,
